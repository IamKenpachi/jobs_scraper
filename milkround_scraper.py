...
async def scrape_milkround(search_query="graduate data analyst", headless=True):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not input_found:
                logger.error("Could not find search bar. Dumping HTML for inspection.")
                # We could dump html here if debug mode is enabled
                await browser.close()
                return
            ...
            logger.info(f"Found {len(job_articles)} potential job listings on Milkround.")
            ...
            for article in job_articles:
                ...
                try:
                    job_model = JobListing(
                        ...
                        salary="",
                        deadline="",
                        url=job_url or "https://www.milkround.com",
            ...
            logger.error(f"Error during Milkround scraping: {e}")
        finally:
            await browser.close()
